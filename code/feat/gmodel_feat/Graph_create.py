...
def Graph_create(piRNA_disease, piRNA_feat, disease_feat):
    graph = {
        ('piRNA', 'p_d', 'disease'): (torch.tensor(piRNA_disease[0]), torch.tensor(piRNA_disease[1])),
        ('disease', 'd_p', 'piRNA'): (torch.tensor(piRNA_disease[1]), torch.tensor(piRNA_disease[0])),
    }

    graph = dgl.heterograph(graph)

    graph.nodes['piRNA'].data['feature'] = piRNA_feat
    graph.nodes['disease'].data['feature'] = disease_feat

    graph_h = {'piRNA': graph.nodes['piRNA'].data['feature'],
