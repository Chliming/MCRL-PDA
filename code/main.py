...
def cross_val(X, y, n_epochs, fold_num, learning_rate, in_dim_mlp, hidden_dim_mlp, dropout, device):
    ...
    for i, (train_index, val_index) in enumerate(kfold.split(X)):
        ...
        print('--' * 40)
        metric_tmp_train = get_metrics(y_train, train_logits)
        metric_tmp_val = get_metrics(y_val, val_logits)
...
def main():
    ...
    # 上传数据
    data = pd.read_csv(r'..\data\MNDRv3.0\feat_final_ass.csv', index_col=0)
    ...
    dropout = 0.5
    # 交叉验证
    in_dim_mlp = 4319  # or 4370 for piRDiseasev1.0
